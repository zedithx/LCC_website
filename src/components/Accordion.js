...
export default function Accordion(props){
    ...
    return (
        <div className='bg-[#596461] rounded-2xl w-11/12 mx-auto'>
            ...
            <div className={`flex flex-wrap gap-4 pb-4 justify-around transition-all duration-500 ease-in-out ${accordionOpen ? 'opacity-100' : 'opacity-0'}`}>
            {accordionOpen &&
            booths.map((booth) => {
                return(
                    <>
                        <div className='bg-[#420059] border-4 px-4 pt-4 pb-10 border-gray-600 w-5/12'>
                            ...
                            <div className='font-gotham text-[#D587FA] pt-2 text-[1.8vw]'>
                                <h1 className='float-left'>{booth.title}</h1>
                                <h1 className='float-right'>{booth.number}</h1>
                            </div>
